...
import { BUCIN_QUESTIONS } from "@/lib/bucin-questions";
import type { BucinQuestion } from "@/lib/bucin-questions";
import { useBucinStore } from "@/lib/store/bucinStore";
import { calculateBucinScore, getBucinTier } from "@/lib/bucin-scoring";
import { saveCompletedSession } from "@/lib/sessionTracking";

const questions = BUCIN_QUESTIONS;
...
export default function BucinQuizPage() {
  const router = useRouter();
  const store = useBucinStore();
  ...
  useEffect(() => {
    async function loadQuestions() {
      try {
        ...
        // Map data to BucinQuestion type if necessary
        const dbQuestions: BucinQuestion[] = (data || []).map((q: any) => ({
          id: Number(q.id),
          teks: q.teks,
          tipe: q.tipe as any,
          kategori: q.kategori as any,
          bobot: Number(q.bobot),
          opsi_jawaban: q.opsi_jawaban as any,
        }));

        const finalQuestions = dbQuestions.length > 0 ? dbQuestions : questions;

        const savedUsername = localStorage.getItem("bucin_username");

        if (store.hasPendingSession) {
          setShowResume(true);
        } else if (!store.sessionId || store.finished) {
          if (!savedUsername) {
            router.replace("/bucin/username");
            return;
          }
          store.initSession(savedUsername, finalQuestions);
        }
      } catch (err) {
        ...
        if (!store.sessionId || store.finished) {
          if (savedUsername) store.initSession(savedUsername, questions);
        }
      } finally {
  ...
  function handleRestart() {
    const savedUsername =
      localStorage.getItem("bucin_username") || store.username;
    store.resetSession();
  ...
  function handleAnswer(nilai: number) {
    ...
    // Auto-advance ke soal berikutnya jika belum di soal terakhir
  ...
  async function handleFinish() {
    ...
    const score = calculateBucinScore(store.answers, store.questions);
    const tier = getBucinTier(score);
    ...
    localStorage.setItem(`bucin_result_${hash}`, JSON.stringify(resultData));
    localStorage.setItem("bucin_active_result_hash", hash);

    router.push(
      `/bucin/result/${hash}?score=${score}&username=${encodeURIComponent(store.username)}&tier=${tier.tier}`
    );
    ...
    saveCompletedSession({
      ...
      finishedAt,
      durationSeconds,
      quizType: "bucin-purity-test",
    });
  }

  // ── Loading state ──
  if (!hydrated || loadingQuestions || (!showResume && store.questions.length === 0)) {
    return (
      <div
              ...
              <button
                onClick={() => {
                  if (!allAnswered) {
                    ...
                    const firstUnansweredIndex = store.questions.findIndex(
                      (q) => store.answers[q.id] === undefined
                    );
                    if (firstUnansweredIndex !== -1) {
                      alert(`Masih ada soal yang terlewat (Nomor ${firstUnansweredIndex + 1}). Yuk dijawab dulu! 💕`);
                      store.navigate(firstUnansweredIndex);
                    }
                    return;
                  }
                  handleFinish();
                ...
                className="px-6 py-3 rounded-2xl font-black text-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 min-w-[120px]"
                style={{
                  backgroundColor: allAnswered && !isSubmitting
                    ? "var(--color-accent)"
                    : "var(--color-border)",
                }}
                aria-label={isSubmitting ? "Sedang memproses kuis..." : "Selesaikan kuis"}
              >
                {isSubmitting ? (
