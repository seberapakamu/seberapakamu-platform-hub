...
function TiersTab({ onToast }: { onToast: (msg: string, type: "success" | "error") => void }) {
  ...
  useEffect(() => {
    fetch("/api/admin/site-content?keys=tier_1,tier_2,tier_3,tier_4,tier_5&module=bucin")
      .then((r) => r.json())
      .then(({ data }) => {
        if (!data) return;
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/site-content?module=bucin", {
        method: "PUT",
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>
          Edit judul, emoji, deskripsi, dan range skor tiap tier bucin.
        </p>
...
function LandingTab({ onToast }: { onToast: (msg: string, type: "success" | "error") => void }) {
  ...
  useEffect(() => {
    fetch("/api/admin/site-content?keys=landing_hero,landing_cta,landing_how_it_works,landing_quiz_list,landing_stats_offset&module=bucin")
      .then((r) => r.json())
      .then(({ data }) => {
        if (!data) return;
...
function WikiTab({ onToast }: { onToast: (msg: string, type: "success" | "error") => void }) {
  ...
  useEffect(() => {
    fetch("/api/admin/site-content?keys=wiki_faqs,wiki_memes,wiki_timeline&module=bucin")
      .then((r) => r.json())
      .then(({ data }) => {
        if (!data) return;
        const get = (key: string) => data.find((d: { key: string; value: unknown }) => d.key === key)?.value;
...
function TentangTab({ onToast }: { onToast: (msg: string, type: "success" | "error") => void }) {
  ...
  useEffect(() => {
    fetch("/api/admin/site-content?keys=tentang_intro,tentang_tiers,tentang_platform_faqs&module=bucin")
      .then((r) => r.json())
      .then(({ data }) => {
        if (!data) return;
        const get = (key: string) => data.find((d: { key: string; value: unknown }) => d.key === key)?.value;
  ...
  async function save() {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json()).error);
      onToast("Konten tentang bucin berhasil disimpan!", "success");
    } catch (e) {
      onToast(e instanceof Error ? e.message : "Gagal menyimpan", "error");
    } finally {
      setSaving(false);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>Edit konten halaman Tentang bucin.</p>
        <SaveBtn saving={saving} onClick={save} />
      </div>

      {/* Intro */}
      <SectionCard title="🤔 Apa Itu bucin?">
        <div className="space-y-3">
          <div>
      ...
      {/* Tingkatan */}
      <SectionCard title="⭐ Tingkatan bucin">
        <div className="space-y-4">
          {tiers.map((tier, i) => (
            <div key={i} className="rounded-2xl p-4" style={{ backgroundColor: "var(--color-bg)", border: "1px solid var(--color-border)" }}>
...
export default function AdminSiteContentPage() {
  ...
  const tabs: { id: Tab; label: string }[] = [
    ...
    { id: "wiki", label: "📖 Wiki" },
    { id: "tentang", label: "🎌 Tentang bucin" },
  ];
