...
type Kategori = "Komunikasi" | "Pengorbanan" | "Prioritas" | "MediaSosial" | "Keuangan";
...
const KATEGORI_OPTIONS: Kategori[] = ["Komunikasi", "Pengorbanan", "Prioritas", "MediaSosial", "Keuangan"];
...
function TimerConfig() {
  ...
  useEffect(() => {
    fetch("/api/admin/questions?module=bucin")
      .then(() => {})
      .catch(() => {});
    // Load config from Supabase via a simple fetch
    fetch("/api/admin/config?module=bucin")
      .then((r) => r.json())
      .then((json) => {
        if (json.data) {
  ...
  async function handleSave() {
    ...
    await fetch("/api/admin/config", {
      ...
      body: JSON.stringify({ timer_enabled: String(enabled), timer_duration_minutes: duration, _module_slug: "bucin" }),
    });
    setSaving(false);
    setSaved(true);
...
export default function AdminQuestionsPage() {
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/questions?module=bucin");
