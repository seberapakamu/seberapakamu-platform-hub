...
import { BUCIN_TIERS } from "@/lib/bucin-scoring";
...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    admin.from("sessions").select("*", { count: "exact", head: true }).eq("quiz_type", "bucin-purity-test"),
    admin.from("sessions").select("*", { count: "exact", head: true }).eq("quiz_type", "bucin-purity-test").not("finished_at", "is", null),
    admin.from("sessions").select("*", { count: "exact", head: true }).eq("quiz_type", "bucin-purity-test").eq("share_clicked", true).not("finished_at", "is", null),
    admin.from("sessions").select("tier").eq("quiz_type", "bucin-purity-test").not("tier", "is", null),
    admin.from("sessions").select("started_at, finished_at").eq("quiz_type", "bucin-purity-test").not("finished_at", "is", null),
  ]);
